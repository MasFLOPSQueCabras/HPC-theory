...
export const BackendDetail: React.FC = () => {
  ...
  return (
    <div className="hpc-card p-5 w-full max-w-5xl mx-auto bg-[#0f131d] border border-[#e6ff00]/30 shadow-2xl">
      {/* Selector Tabs */}
        ...
        {blocks.map((b, idx) => (
          <button
            ...
            className={`p-2.5 rounded-lg text-center transition-all cursor-pointer border ${
              selectedBlock === idx
                ? 'bg-[#151a27] border-[#e6ff00] text-[#e6ff00] shadow-md font-bold'
      ...
      <div className="p-4 rounded-xl bg-[#07080c] border border-[#232a3d]">
        <div className="flex items-center justify-between mb-2">
          ...
          <span className="hpc-badge-yellow font-mono text-[10px] mb-0">{current.badge}</span>
        </div>
        <p className="m-0 text-xs text-slate-300 leading-relaxed mb-3">{current.detail}</p>
        <div className="p-2 rounded bg-[#0f131d] border border-[#232a3d] text-[11px] font-mono text-[#e6ff00]">
          {current.specs}
        </div>
      </div>
    </div>
