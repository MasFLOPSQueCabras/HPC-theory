import React from 'react';

export const AmdCuOverview: React.FC = () => {
  return (
    <div style={{ width: '100%', maxWidth: '960px', margin: '0 auto' }}>
      {/* Full-Width Panoramic SVG: CU Diagram */}
      <div style={{ background: '#070a12', padding: '0.8rem 1rem', borderRadius: '10px', border: '1px solid rgba(248, 113, 113, 0.3)', marginBottom: '0.8rem' }}>
        <svg viewBox="0 0 740 220" style={{ width: '100%', height: 'auto', display: 'block' }}>
          {/* Outer CU Box */}
          <rect x="5" y="5" width="730" height="210" rx="8" fill="#0b0f19" stroke="#f87171" strokeWidth="1.8" />

          {/* Top LDS */}
          <rect x="18" y="14" width="704" height="28" rx="5" fill="rgba(248, 113, 113, 0.15)" stroke="#f87171" strokeWidth="1.2" />
          <text x="370" y="32" fill="#f87171" fontSize="11" fontWeight="bold" textAnchor="middle">
            LDS (Local Data Share: 64 KB - 128 KB SRAM Ultra-Rápida compartida en el CU)
          </text>

          {/* 4 Spacious SIMD Units */}
          {[0, 1, 2, 3].map((simd) => (
            <g key={simd}>
              {/* SIMD Unit Outer Boundary */}
              <rect x={18 + simd * 178} y="48" width="168" height="155" rx="6" fill="#1e293b" stroke="rgba(255,255,255,0.15)" strokeWidth="1" />
              
              {/* Header Wavefront Scheduler */}
              <rect x={24 + simd * 178} y="54" width="156" height="20" rx="4" fill="#070a12" stroke="#f87171" strokeWidth="1" />
              <text x={102 + simd * 178} y="68" fill="#ffffff" fontSize="8.5" fontWeight="bold" textAnchor="middle">
                Wave Scheduler (Wave32/64)
              </text>

              {/* Dual Registers: VGPR + SGPR */}
              <rect x={24 + simd * 178} y="78" width="156" height="20" rx="4" fill="rgba(244, 184, 96, 0.15)" stroke="#f4b860" strokeWidth="1" />
              <text x={102 + simd * 178} y="92" fill="#f4b860" fontSize="8.5" fontWeight="bold" textAnchor="middle">
                Dual RF: VGPR (Vec) + SGPR (Scalar)
              </text>

              {/* Middle Compute Section: VALU Vector + Matrix Cores */}
              <rect x={24 + simd * 178} y="103" width="75" height="46" rx="4" fill="#070a12" stroke="rgba(52,211,153,0.4)" strokeWidth="1.2" />
              <text x={61 + simd * 178} y="120" fill="#34d399" fontSize="9" fontWeight="bold" textAnchor="middle">16-32 VALU</text>
              <text x={61 + simd * 178} y="132" fill="#ffffff" fontSize="7.5" textAnchor="middle">Vector Cores</text>
              <text x={61 + simd * 178} y="143" fill="#94a3b8" fontSize="7" textAnchor="middle">FP32 / FP64</text>

              <rect x={105 + simd * 178} y="103" width="75" height="46" rx="4" fill="#070a12" stroke="rgba(56,189,248,0.4)" strokeWidth="1.2" />
              <text x={142 + simd * 178} y="120" fill="#38bdf8" fontSize="9" fontWeight="bold" textAnchor="middle">1 Matrix</text>
              <text x={142 + simd * 178} y="132" fill="#ffffff" fontSize="7.5" textAnchor="middle">Core (MFMA)</text>
              <text x={142 + simd * 178} y="143" fill="#94a3b8" fontSize="7" textAnchor="middle">BF16/FP8/MXFP4</text>

              {/* Bottom SALU & L1 Cache */}
              <rect x={24 + simd * 178} y="154" width="75" height="22" rx="3" fill="#070a12" stroke="rgba(255,255,255,0.12)" />
              <text x={61 + simd * 178} y="169" fill="#cbd5e1" fontSize="7.5" fontWeight="bold" textAnchor="middle">SALU (Scalar ALU)</text>

              <rect x={105 + simd * 178} y="154" width="75" height="22" rx="3" fill="#070a12" stroke="rgba(255,255,255,0.12)" />
              <text x={142 + simd * 178} y="169" fill="#cbd5e1" fontSize="7.5" fontWeight="bold" textAnchor="middle">L1 Vector Cache</text>

              {/* Unit Label */}
              <text x={102 + simd * 178} y="193" fill="#94a3b8" fontSize="8" textAnchor="middle">
                SIMD Unit {simd}
              </text>
            </g>
          ))}
        </svg>
      </div>

      {/* 2 Bottom Explanatory Cards */}
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.8rem', textAlign: 'left' }}>
        <div className="hpc-card" style={{ padding: '0.8rem 1.1rem' }}>
          <strong style={{ fontSize: '0.86rem', color: '#f87171' }}>4 Unidades SIMD por Compute Unit</strong>
          <p style={{ margin: '0.2rem 0 0 0', fontSize: '0.78rem', color: '#cbd5e1', lineHeight: 1.4 }}>
            Cada CU ejecuta Wavefronts de 32 o 64 hilos, alternando flujos vectoriales y escalares para saturar las ALUs en cada ciclo.
          </p>
        </div>

        <div className="hpc-card" style={{ padding: '0.8rem 1.1rem' }}>
          <strong style={{ fontSize: '0.86rem', color: '#38bdf8' }}>Agrupamiento WGP (Workgroup Processor)</strong>
          <p style={{ margin: '0.2rem 0 0 0', fontSize: '0.78rem', color: '#cbd5e1', lineHeight: 1.4 }}>
            En arquitecturas AMD modernas, <strong>2 Compute Units se fusionan en un WGP</strong>, compartiendo cachés L1 de instrucciones y datos para duplicar el rendimiento en cómputo denso.
          </p>
        </div>
      </div>
    </div>
  );
};
